    ...
    protected void izvrsiKonkretnuOperaciju() throws Exception {
        ...
        for (AbstractObject abstractO : izBaze) {
            ...
            if (s.getRezervacijaID().toLowerCase().contains(pretraga)
                    || s.getSkijas().getIme().toLowerCase().contains(pretraga)
                    || s.getSkijas().getPrezime().toLowerCase().contains(pretraga)
                    || (s.isUplataUnapred() && pretraga.equalsIgnoreCase("Da"))
                    || (!s.isUplataUnapred() && pretraga.equalsIgnoreCase("Ne"))
